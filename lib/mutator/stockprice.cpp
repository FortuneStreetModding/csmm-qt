...
StockPrice::StockPrice(const YAML::Node &yaml, bool enabled) : Mutator(StockPriceType, enabled) {
    if(yaml["numerator"])
...
StockPrice::StockPrice(QDataStream &stream, bool enabled) : Mutator(StockPriceType, enabled) {
    stream >> numerator;
    stream >> denominator;
    stream >> constant;
}
